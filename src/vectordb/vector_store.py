...
class QdrantVectorStore:
    def __init__(self, embedder: APIEmbedder, url: str = "http://localhost:6333", collection_name="vietnam_law_corpus",
        ...
        if not self.client.collection_exists(collection_name=self.collection_name):
            ...
            print(f"Collection '{self.collection_name}' đã được tạo thành công trong Qdrant.")
        else:
    ...
    def process_and_chunk_law_json(self, json_file_path):
        ...
        for law_item in data:
            ...
            for article in articles:
                ...
                enriched_content = f"[Law ID: {law_id}] [Article ID: {aid}]\n{raw_content}"
    ...
    def insert_chunks_to_vector_db(self, chunks):
        ...
        assert len(embeddings) == len(texts_to_embed), (
            f"Expected {len(texts_to_embed)} embeddings, "
